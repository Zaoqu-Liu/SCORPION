#' Run PANDA algorithm for gene regulatory network inference
#' @description Internal function implementing the PANDA message passing algorithm
#' @param motif Motif prior network (TF-gene interactions)
#' @param expr Gene expression matrix
#' @param ppi Protein-protein interaction network
#' @param alpha Learning rate for network updates
#' @param hamming Convergence threshold based on Hamming distance
#' @param n.cores Number of cores for parallel computation
#' @param iter Maximum number of iterations
#' @param output Which networks to return
#' @param zScale Whether to Z-scale output
#' @param progress Show progress messages
#' @param randomize Randomization method for null model
#' @param assoc.method Association method for co-expression
#' @param scale.by.present Scale by detection rate
#' @param edgelist Return as edge list
#' @param remove.missing.ppi Remove TFs not in PPI
#' @param remove.missing.motif Remove genes not in motif
#' @param remove.missing.genes Remove genes not in expression
#' @param mode Gene/TF matching mode
#' @param computing.engine Computation engine (cpu/gpu)
#' @return List of inferred networks
#' @import cli
#' @noRd
runPANDA <- function(motif = NULL, expr = NULL, ppi = NULL, alpha = 0.1, hamming = 0.001, n.cores = 1,
  ...
  if (is.null(expr)) {
    ...
    if (mode == "legacy") {
      ...
      expr1[which(gene.names %in% rownames(expr)), ] <- as.matrix(expr)
      expr <- expr1
      # PPI matrix
  ...
  # Check for NA and replace them by zero - compatible with both sparse and dense
  if (inherits(geneCoreg, "sparseMatrix")) {
    ...
    if (any(is.na(geneCoreg))) {
      ...
      geneCoreg <- Matrix::Matrix(geneCoreg)
    }
  }
